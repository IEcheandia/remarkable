...
AppMain::AppMain():
	...
	m_dbNotificationServer(m_oDbProxy),
	m_dbNotificationHandler(&m_dbNotificationServer),
	m_control(inspectProxy_, inspectCmdProxy_, m_oSensorProxy, m_oEthercatOutputsProxy, m_oS6K_InfoToProcessesProxy, m_oDbProxy, &m_dbNotificationServer),
    ...
    m_postProcessingController(
        ...
        &centralDeviceManager_,
        true),
    m_postProcessingTrigger(&m_postProcessingController, m_oDbProxy),
    m_storageServer(std::make_unique<StorageUpdateServer>()),
    m_storageHandler(std::make_unique<TStorageUpdate<EventHandler>>(m_storageServer.get()) )
{
    ...
    registerSubscription(&m_dbNotificationHandler);
    
    registerSubscription(m_storageHandler.get());
	...
	if(m_simulateHW)
	{
		...
		while(!markedForExit()){
			...
			else if (choose == "4")
			{
                ...
                std::printf("\t(3) Start PostProcessing Test \n");
				char choose2 = '0';
				std::cin >> choose2;
				switch (choose2) {
					case '1':{
                    ...
                    case '3':{
                        m_postProcessingTrigger.testHandler();
                        break;
                    }
					default:{
						std::printf("Invalid command!\n");
