...
// wm includes
#include "../include/viInspectionControl/postProcessingController.h"
...
bool PostProcessingController::activate(const UUID& p_rProductId, int p_oProductNb, const std::string &p_rExtendedProductInfo) {
    ...
    std::cout << "Activate Product " << p_oProductNb << std::endl;
...
std::cout << "active product initialized" << std::endl;
...
bool PostProcessingController::activateSeamSeries(int p_oSeamseries) {
  //  std::cout << "activate seam series" << std::endl;
	const Poco::ScopedLock<Poco::FastMutex> oScopedLock{m_oManSync};
...
{
  //  std::cout << "about to read product" << std::endl;
	const auto	oSeamSeriesFound	= m_pActiveProduct->m_oSeamSeries.find(p_oSeamseries);
 //   std::cout << "ActiveProduct->seamseries: " << oSeamSeriesFound->second.m_oNumber << std::endl;
    ...
    std::cout << "seamseries found" << std::endl;
...
bool PostProcessingController::activateSeam(int p_oSeam, const std::string &seamLabel) {
   // const auto oScopedLock = atomLock( &m_oManSync ); // no lock, because called internally from startInspect or startCrosswiseAction
    ...
    std::cout << "Activate seam" << p_oSeam << std::endl;
...
bool PostProcessingController::startInspect(int p_oSeamseries, int p_oSeam, const std::string &label) {
    ...
    std::cout << "Start inspection..." << std::endl;
    sleep(3);
	const Poco::ScopedLock<Poco::FastMutex> oScopedLock{m_oManSync};
...
void PostProcessingController::stopInspect() {
    std::cout << "Stopping current inspection" << std::endl;
    const Poco::ScopedLock<Poco::FastMutex> oScopedLock{m_oManSync};
    stopInspectInternal();
} // stopInspect
...
void PostProcessingController::stopInspectInternal()
{
   ...
   // std::cout << "stopInspectInternal" << std::endl;
     ...
     //   std::cout << "there is an active graph" << std::endl;
		auto		oActiveGraphAssistent	=	GraphAssistent( m_pActiveGraph );
...
bool PostProcessingController::processImage(int p_oSensorId, const TriggerContext& p_rTriggerContext, const BImage& p_rImage, ProcessingMode mode) {
    ...
    std::cout << "Second Image processing" << std::endl;
	...
	{
        std::cout << "No all samples queued" << std::endl;
		if (m_oImageQueue.size() < 3)
		{
            ...
            std::cout << "Image queue less than 3" << std::endl;
			return false;
		}
...
bool PostProcessingController::processSample(int p_oSensorId, const TriggerContext& p_rTriggerContext, const Sample& p_rSample) {
    ...
    std::cout << "Second sample processing " << std::endl;
	//	1)	enqueue new frame
	//
...
{
    std::cout << "Image starting processing..." << std::endl;
    sleep(5);
    ...
    {
     //   std::cout << "work scheduled" << std::endl;
        m_lastProcessedImage = triggerContext.imageNumber();
        ++m_oNbSeamSignaled;
        std::cout << "SignalAdapter scheduled" << std::endl;
    }
    else
    {
