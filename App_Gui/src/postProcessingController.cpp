...
typedef interface::ResultType ResultType;
...
static const auto	g_oPrePosDisabled		= 60354318;	// magic number - signals 'no prepositioning', see 'public const Int32 MessaufgabeDisabledValue = 60354318;' in MessaufgabeVM.cs
...
PostProcessingController::PostProcessingController(
		...
		if_tinspection_out_t*	p_pInspectionOutProxy,
...
//	m_oResultHandlerCWAction		{ p_pTriggerCmdProxy, p_pInspectionOutProxy, p_pWeldHeadMsgProxy },
...
{
    std::cout << "PostProcessingController::PostProcessingController() COTR." << std::endl;
 //   sleep(5);
#ifndef NDEBUG
	...
	for (size_t i = 0; i < Environment::processorCount() && i < g_oNbParMax; i++)
	{
		...
		m_oWorkers.at(i).setWorkDoneCallback(std::bind(&PostProcessingController::processingThreadFinishedCallback, this));
	}
...
DeviceParameter& PostProcessingController::getDeviceParameter()
{
...
void PostProcessingController::changeProduct(const interface::Product& p_rProduct) {
    ...
    wmLog( eDebug, "PostProcessingController::changeProduct() getProductParameter returns oProdParams.size() <%d>\n", oProdParams.size());
...
bool PostProcessingController::deleteSumError(const UUID& p_rProductID, const UUID& p_rSumErrorID) {
	const Poco::ScopedLock<Poco::FastMutex> oScopedLock{m_oManSync};
...
bool PostProcessingController::activate(const UUID& p_rProductId, int p_oProductNb, const std::string &p_rExtendedProductInfo) {
	const Poco::ScopedLock<Poco::FastMutex> oScopedLock{m_oManSync};
...
bool PostProcessingController::activateSeamSeries(int p_oSeamseries) {
	const Poco::ScopedLock<Poco::FastMutex> oScopedLock{m_oManSync};
    return activateSeamSeriesInternal(p_oSeamseries);
} // activateSeamSeries

bool PostProcessingController::activateSeamSeriesInternal(int p_oSeamseries)
{
...
bool PostProcessingController::activateSeam(int p_oSeam, const std::string &seamLabel) {
   // const auto oScopedLock = atomLock( &m_oManSync ); // no lock, because called internally from startInspect or startCrosswiseAction
...
void PostProcessingController::changeFilterParameter(const UUID& p_rMeasureTaskID, const UUID& p_rInstanceFilterId) {
    ...
    wmLog( eDebug, "PostProcessingController::changeFilterParameter() start with oParameterList.size() <%d>\n", oParameterList.size());
...
/*
void PostProcessingController::startCrosswiseAction(int p_oSeamseries, int p_oSeam) {
	const Poco::ScopedLock<Poco::FastMutex> oScopedLock{m_oManSync};

	poco_assert(nullptr != m_pActiveProduct);
	wmLogTr( eInfo, "QnxMsg.Workflow.SSearchStart", "Crosswise action started.\n");

	activateSeamSeriesInternal(p_oSeamseries);	// seam series not found case should not occur here
	activateSeam(p_oSeam);				// seam not found case should not occur here
	...
	m_pActiveGraph			=  m_pActiveSeam->getGraph();		// take graph from seam

	const auto&		rActiveMeasureTask		=	*m_pActiveSeam->m_pMeasureTask;	;	// take task from seam
    ...
    updateHardwareData(m_pActiveGraph);
    oActiveGraphAssistent.setExternalData(m_externalHardwareData);
	oActiveGraphAssistent.setExternalData(m_oExternalProductData);			// nb: do this before arm of seam start and setParameterSet

    if (m_lastSetParameterSet != rParamSetId)
    {
        oActiveGraphAssistent.setParameterSet(rParamSetId, m_oParameterSetMap);	// will be done again on seam interval change, but might be needed in arm functions
        m_lastSetParameterSet = rParamSetId;
    }
	oActiveGraphAssistent.arm(eSeamStart);						// after setExternalData, so external product data is already set in filters

	oActiveGraphAssistent.releaseResultHandler(m_oResultHandlerCWAction);
	oActiveGraphAssistent.setResultHandler(m_oResultHandlerCWAction);

	m_pActiveSeamInterval					= &m_pActiveSeam->m_oIntervalCWAction;

	armHwParameters(rHwParamSetId);
	wmLog(eDebug, "HW armed on crosswise action level: '%s'\n", rHwParamSetId.toString().c_str());

	// do prepositioning if enabled

	const auto	oPrePositioningPos	=	rActiveMeasureTask.start();
	const auto	oIsPrePosEnabled	=	oPrePositioningPos	!=	g_oPrePosDisabled;

	if (oIsPrePosEnabled == true) {
		const auto			oHeadMoved			=	m_pWeldHeadMsgProxy->setHeadPos(eAxisY, oPrePositioningPos);

		if (oHeadMoved == true) {
			wmLog(eDebug, "Prepositioning - head moved to Y = %i um.\n", oPrePositioningPos);
		} // if
		else {
			//eAxis
			wmFatal(eAxis, "QnxMsg.Fatal.YAxisNotMoved", "Y-axis has not moved to Y = %i um.\n", oPrePositioningPos);

			return;
		} // else
	} // if

	// if no graph set only prepositioning, no crosswise action

	if (rActiveMeasureTask.graphID() == g_oDefaultGraphID) {
		return;
	} // if

	updateProductInfo();
	m_pInspectionOutProxy->setSearchSeamActive(true);
	m_oResultHandlerCWAction.init(rSensorIds); // get current y axis parameters
	// if prepositioning is disabled, move the y axis to the lower soft limit switch (agreement)

	if (oIsPrePosEnabled == false) {
		const auto			oAxisYLowerLimit	=	m_oResultHandlerCWAction.getAxisYLowerLimit();	///< [um], get limit with buffer. NB: 'init()' needs to have been called before.
		const auto			oHeadMoved			=	m_pWeldHeadMsgProxy->setHeadPos(eAxisY, oAxisYLowerLimit);
		if (oHeadMoved == true) {
			wmLog(eDebug, "Head moved to lower limit = %i um.\n", oAxisYLowerLimit); // debug
		} // if
		else {
			wmFatal(eAxis, "QnxMsg.Fatal.YAxisNotMoved", "Y-axis has not moved to Y = %i um.\n", oAxisYLowerLimit);
		} // else
	...
	getTriggerCmdSrv().single(rSensorIds, TriggerContext( 0, 0, 0 ));
} // startCrosswiseAction

*/

// Vorstart der Inspektion. Dazu wird die Naht gesucht, welche jetzt inspiziert werden soll
bool PostProcessingController::seamPreStart(int p_oSeamseries, int p_oSeam) {
	const Poco::ScopedLock<Poco::FastMutex> oScopedLock{m_oManSync};
...
bool PostProcessingController::startInspect(int p_oSeamseries, int p_oSeam, const std::string &label) {
	const Poco::ScopedLock<Poco::FastMutex> oScopedLock{m_oManSync};

	try {
        std::cout << "POSTPROCESSING_CONTROLLER::StartInspection(); Start the inspection" << std::endl;
		poco_assert(nullptr != m_pActiveProduct);
 ...
 //       std::cout << "POSTPROCESSING_CONTROLLER::StartInspection(); Getting sensors ids" << std::endl;
    ...
    //    std::cout << "POSTPROCESSING_CONTROLLER::StartInspection(); Update Seam Interval" << std::endl;
  ...
  //  std::cout << "POSTPROCESSING_CONTROLLER::StartInspection(); End of start inspection" << std::endl;
    sleep(1);
	return true;
} // startInspect

void PostProcessingController::triggerSingle(const interface::TriggerContext &context)
{
    const Poco::ScopedLock<Poco::FastMutex> oScopedLock{m_oManSync};
    getTriggerCmdSrv().single(m_oSensorIds, context);
}

void PostProcessingController::stopInspect() {
    const Poco::ScopedLock<Poco::FastMutex> oScopedLock{m_oManSync};
    stopInspectInternal();
} // stopInspect

void PostProcessingController::stopInspectInternal()
{
	try {
...
bool PostProcessingController::startSeamseries(int p_oSeamseries) {
	const Poco::ScopedLock<Poco::FastMutex> oScopedLock{m_oManSync};
...
void PostProcessingController::data(int p_oSensorId, const TriggerContext& p_rTriggerContext, const BImage& p_rImage) {
    system::ElapsedTimer timer;
        const Poco::ScopedLock<Poco::FastMutex> oScopedLock{m_oManSync};
     
        
        std::cout << "POSTPROCESSING_CONTROLLER::dataImage(); Starting image data porcessing in Controller" << std::endl;
        
       // std::cout << "POSTPROCESSING_CONTROLLER::dataImage(); Image number: " << p_rTriggerContext.imageNumber() << std::endl;
      //  std::cout << "POSTPROCESSING_CONTROLLER::dataImage(); Image timestamp: " << p_rTriggerContext.relativeTime() << std::endl;
 ...
 //   std::cout << "POSTPROCESSING_CONTROLLER::dataImage(); Updating seam position" << std::endl;
 ...
 //   std::cout << "POSTPROCESSING_CONTROLLER::dataImage(); Processing image" << std::endl;
    sleep(1);
...
void PostProcessingController::data(int p_oSensorId, const TriggerContext& p_rTriggerContext, const Sample& p_rSample) {
    system::ElapsedTimer timer;
    
    std::cout << "POSTPROCESSING_CONTROLLER::dataSample(); Starting sample data processing in the controller" << std::endl;
    sleep(1);
    const Poco::ScopedLock<Poco::FastMutex> oScopedLock{m_oManSync};
  ...
  //  std::cout << "POSTPROCESSING_CONTROLLER::dataSample(); Updating seam interval" << std::endl;
	checkSeamIntervalChange(p_rTriggerContext.imageNumber());

 //   std::cout << "POSTPROCESSING_CONTROLLER::dataSample(); Processing sample" << std::endl;
    sleep(1);
...
void PostProcessingController::updateProductInfo() {
...
void PostProcessingController::startLiveMode() {
	const Poco::ScopedLock<Poco::FastMutex> oScopedLock{m_oManSync};
...
void PostProcessingController::stopLiveMode() {
	const Poco::ScopedLock<Poco::FastMutex> oScopedLock{m_oManSync};
...
void PostProcessingController::startAutomaticMode() {
	const Poco::ScopedLock<Poco::FastMutex> oScopedLock{m_oManSync};
...
void PostProcessingController::stopAutomaticMode() {
	const Poco::ScopedLock<Poco::FastMutex> oScopedLock{m_oManSync};
...
void PostProcessingController::setState(State p_oState) {
	const Poco::ScopedLock<Poco::FastMutex> oScopedLock{m_oManSync};
...
void PostProcessingController::updateHwParameter( const UUID &p_rParamSetId, const Key p_oKey )
{
	const Poco::ScopedLock<Poco::FastMutex> oScopedLock{m_oManSync};
...
void PostProcessingController::updateHardwareData(const FilterGraph* graph)
{
...
void PostProcessingController::setNoHWParaAxis( bool p_oNoHWParaAxis)
{
...
bool PostProcessingController::getNoHWParaAxis()
{
...
void PostProcessingController::resetSumErrors() {
	const Poco::ScopedLock<Poco::FastMutex> oScopedLock{m_oManSync};
...
math::CalibrationData & PostProcessingController::getCalibrationData(const int p_oSensorID)
{
...
bool PostProcessingController::loadCalibDataAfterSignal(int p_oSensorID, bool p_oInit )
{
    wmLog(eDebug, "PostProcessingController::loadCalibDataAfterSignal(sensorId %d, init %d ) \n", p_oSensorID, p_oInit);
...
bool PostProcessingController::reloadCalibData(int p_oSensorID, math::Calibration3DCoords p_o3DCoords, math::CalibrationParamMap p_oCalibrationParameters)
{
    wmLog(eDebug, "PostProcessingController::reloadCalibData\n");
...
bool PostProcessingController::updateCorrectionGrid(int p_oSensorID, coordinates::CalibrationCameraCorrectionContainer p_oCameraCorrectionContainer, coordinates::CalibrationIDMCorrectionContainer p_oIDMCorrectionContainer)
{
    math::CalibrationData &rCalibData = getCalibrationData(p_oSensorID);
...
bool PostProcessingController::assertCalibData(int p_oSensorID)
...
graph_map_t::const_iterator PostProcessingController::storeGraphAndParamterSet(const Poco::UUID &p_rMeasureTaskId, const Poco::UUID &p_rGraphId,  const Poco::UUID &p_rParamSetId) {
...
graph_map_t::const_iterator PostProcessingController::cacheGraph(const UUID &p_rMeasureTaskId, const UUID &p_rGraphId) {
...
void PostProcessingController::cacheParamSet(const UUID& p_rParamSetId, const FilterGraph* p_pGraph) {
...
void PostProcessingController::changeSeamInterval(int p_oActualPos) {
...
void PostProcessingController::checkSeamIntervalChange(int p_oImageNumber) {
...
bool PostProcessingController::processImage(int p_oSensorId, const TriggerContext& p_rTriggerContext, const BImage& p_rImage, ProcessingMode mode) {
#if !defined(NDEBUG)
	wmLog(eDebug, "\t%s::BImage[%i]\t%i\n", __FUNCTION__, p_oSensorId, p_rTriggerContext.imageNumber());
#endif
	poco_assert_dbg(m_pActiveGraph != nullptr);

  //  std::cout << "POSTPROCESSING_CONTROLLER::processImage(); Starting image processing in the controller" << std::endl;
 // std::cout << "POSTPROCESSING_CONTROLLER::processImage(); image number: " << p_rTriggerContext.imageNumber() << std::endl;
	//	1)	enqueue new frame
	//
 ...
 //   std::cout << "POSTPROCESSING_CONTROLLER::processImage(); Starting processing in the worker thread" << std::endl;
...
bool PostProcessingController::processSample(int p_oSensorId, const TriggerContext& p_rTriggerContext, const Sample& p_rSample) {
#if !defined(NDEBUG)
	wmLog(eDebug, "\t%s::Sample[%i]\t%i\n", __FUNCTION__, p_oSensorId, p_rTriggerContext.imageNumber());
#endif
	poco_assert_dbg(m_pActiveGraph != nullptr);

 //   std::cout << "POSTPROCESSING_CONTROLLER::processSample(); Starting sample processing in the controller" << std::endl;
  ...
  //  std::cout << "POSTPROCESSING_CONTROLLER::processSample(); Starting processing in the worker thread" << std::endl;
	startProcessing(oIdxWorkerCur);
    return true;
} // processSample

void PostProcessingController::startProcessing(size_t oIdxWorkerCur)
{
    std::cout << "POSTPROCESSING_CONTROLLER::startProcessing(); Starting processing on the thread worker" << std::endl;
	static uint32_t imageNum = 1;
	...
	signalAdapter->setImageNumber(imageNum);
 ...
 //   std::cout << "POSTPROCESSING_CONTROLLER::startProcessing(); Image number: " << m_oTriggerContexts[oIdxWorkerCur].imageNumber() << std::endl;
 ...
 //   std::cout << "POSTPROCESSING_CONTROLLER::startProcessing(); Processing started with Signal Adapter" << std::endl;


}

void PostProcessingController::queueSample(int sensorId, const TriggerContext &triggerContext, const image::Sample &sample)
{
...
std::map<int, SampleFrame> PostProcessingController::dequeueSamples()
{
...
bool PostProcessingController::areAllSamplesQueued()
{
	if (!m_pActiveGraph)
	{
		return false;
...
void PostProcessingController::armHwParameters( const UUID &p_rParamSetId ) {
...
ImageContext PostProcessingController::makeImageContext(const TriggerContext& p_rTriggerContext) const {
...
void PostProcessingController::releaseDataPipesAndResultHandler(fliplib::FilterGraph* p_pGraph)
{
...
void PostProcessingController::joinWorkers()
{
...
void PostProcessingController::resetCalibration ( int p_oSensorID )
{
...
std::shared_ptr<interface::TRecorderPoll<interface::AbstractInterface>> PostProcessingController::recorderPollServer() const
{
    return m_imageSender;
}

void PostProcessingController::sendLastTimeResult(ResultType oResultType)
{
...
void PostProcessingController::updateInspectManagerTime(const std::chrono::nanoseconds &time, bool increaseFrameCount )
{
    m_oInspectTimer.updateInspectManagerTime(time, increaseFrameCount);

    if (g_oDebugTimings && increaseFrameCount)
    {        
        sendLastTimeResult(interface::InspectManagerTime);
        ...
        sendLastTimeResult(interface::ProcessTime);
    }
}

void PostProcessingController::processingThreadFinishedCallback()
{
    std::lock_guard<std::mutex> lock{m_callbackMutex};
    m_oNbSeamJoined++;
}

void PostProcessingController::simulationDataMissing(const TriggerContext &context)
{
    if (!m_simulationStation)
    {
        return;
...
void PostProcessingController::updateParamSetWihoutDB(storage::ParameterSet* storageParamSet, storage::Parameter* storageParam)
{
    std::cout << "Process controller :: update Param wihtout DB!!" << std::endl;
    auto interfaceFilterParam = storageParam->toFilterParameter();
    
    auto paramUUID = interfaceFilterParam->parameterID();
   
    auto filterQUuid = storageParam->filterId();
    
    auto filterID = convertQUuidToPoco(filterQUuid);
    
    Poco::UUID paramSetID = convertQUuidToPoco(storageParamSet->uuid());
    
    //Find the changed ParameterSet in the map
    auto paramSet = m_oParameterSetMap.find(paramSetID);    
    auto filtersParamsMap = paramSet->second;
    
    //Find the ParameterList of the changed Filter
    auto filterParamsVector = filtersParamsMap.find(filterID)->second;
    
    //get the element with the same parameter ID
    auto it = std::find_if(filterParamsVector.begin(), filterParamsVector.end(), [&paramUUID] ( const std::shared_ptr<precitec::interface::FilterParameter>& filterParam)
    {
        return filterParam->parameterID() == paramUUID;
    });
    
    //Replace the existing param by the new modified one. 
    if( it != filterParamsVector.end())
    {
        (*it) = interfaceFilterParam;
    }
    
    auto task = findTaskFromParameterSets(paramSetID);
    
    auto graphID = task.graphID();
    
    const auto	oGraphFound				=	m_oGraphs.find(graphID);
    ...
    oGraphAssistent.setParameterSet(paramSetID, m_oParameterSetMap);         
    
    m_oReferenceCurves.requestAndStoreRefCurves(filterParamsVector); 
    oGraphAssistent.arm(eSeamStart);
	oGraphAssistent.arm(eSeamIntervalChange);

    wmLogTr( eInfo, "QnxMsg.Workflow.ParamUpdated", "Filter parameter updated.\n");
	m_pSystemStatusProxy->filterParameterUpdated(task.taskID(), filterID);
}

interface::MeasureTask PostProcessingController::findTaskFromParameterSets(Poco::UUID paramSetID)
{
    auto it = std::find_if(m_oMeasureTasks.begin(), m_oMeasureTasks.end(), [&paramSetID] (const std::pair<const Poco::UUID, precitec::interface::MeasureTask>& task)
    {
        return task.second.parametersatzID() == paramSetID;
    });
    
    if( it != m_oMeasureTasks.end())
    {
        return (*it).second;
    }
    return MeasureTask{};
...
Poco::UUID PostProcessingController::convertQUuidToPoco(const QUuid& quuid)
{
