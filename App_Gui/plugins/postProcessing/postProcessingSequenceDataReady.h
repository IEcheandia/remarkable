...
public:
    ...
    ~PostProcessingSequenceDataReady();
    void init();
    ...
    {
        std::cout << "PostProcessingSequenceDataReady getProductList" << std::endl;
        sleep(2);
    ...
    {
        std::cout << "PostProcessingSequenceDataReady setDBProxy, proxy addr: " <<  &dbProxy << std::endl;
        sleep(3);
        std::cout << "-------------------------" << std::endl;
       /* try
        {
        ...
        m_dbNotificationServer.setDBProxy(dbProxy);
        m_referenceCurves.setDbProxy(&dbProxy); // Fix: Pass the address of dbProxy
    }
