...
//#include "analyzer/resultHandlerCrosswiseAction.h"
...
using namespace analyzer;
	...
	public:
		...
		typedef interface::TInspectionOut<interface::AbstractInterface>	if_tinspection_out_t;
        typedef InspectManager::ProcessingMode      ProcessingMode;
  ...
  /*      enum class ProcessingMode {
		 ...
		 * @param	p_pInspectionOutProxy		inspection out event proxy	- signals crosswise action end
...
//		void startCrosswiseAction(int p_oSeamseries, int p_oSeam);
        ...
        void updateParamSetWihoutDB(storage::ParameterSet* storageParamSet, storage::Parameter* storageParam);

		// Setters for proxies

		void setDbProxy(if_tdb_t* p_pDbProxy) {   m_pDbProxy = p_pDbProxy; }
		void setWeldHeadMsgProxy(if_tweld_head_msg_t* p_pWeldHeadMsgProxy) { m_pWeldHeadMsgProxy = p_pWeldHeadMsgProxy; }
		void setTriggerCmdProxy(if_ttrigger_cmd_t* p_pTriggerCmdProxy) { m_pTriggerCmdProxy = p_pTriggerCmdProxy; }
		void setResultProxy(if_tresults_t* p_pResultProxy) { m_pResultProxy = p_pResultProxy; }
		void setRecorderProxy(if_trecorder_t* p_pRecorderProxy) { m_pRecorderProxy = p_pRecorderProxy; }
		void setSystemStatusProxy(if_tsystem_status_t* p_pSystemStatusProxy) { m_pSystemStatusProxy = p_pSystemStatusProxy; }
		void setVideoRecorderProxy(if_tvideo_recorder_t* p_pVideoRecorderProxy) { m_pVideoRecorderProxy = p_pVideoRecorderProxy; }
		void setInspectionOutProxy(if_tinspection_out_t* p_pInspectionOutProxy) { m_pInspectionOutProxy = p_pInspectionOutProxy; }
		void setCentralDeviceManager(CentralDeviceManager* p_pCentralDeviceManager) { m_pCentralDeviceManager = p_pCentralDeviceManager; }
	...
	private:
        ...
        void sendLastTimeResult (interface::ResultType oResultType); 
        
        Poco::UUID convertQUuidToPoco(const QUuid& quuid);
         
        interface::MeasureTask findTaskFromParameterSets(Poco::UUID paramSetID);
		...
		if_tinspection_out_t&			getInspectionOutSrv()	{ return *m_pInspectionOutProxy; }	///< getter proxy
		...
		if_tinspection_out_t* 			m_pInspectionOutProxy; 		///< inspection out event proxy		- signals crosswise action end
		...
		analyzer::Product* 					m_pActiveProduct;			///< points to active product, set in activate()
	...
	//	analyzer::ResultHandlerC     			m_oResultHandlerCWAction;	///< result handler, which handles results stemming from crosswise action
        ...
        std::array<InspectManager::ProcessingMode, 4>     m_oProcessingModes;         ///< The ProcessingMode per worker
